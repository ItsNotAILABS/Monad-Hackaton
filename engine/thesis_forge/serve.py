...
app.include_router(wallet_router); app.include_router(synthetic_router)

@app.middleware("http")
async def engine_prefix_gateway(request:Request,call_next):
    path=request.scope.get("path","")
    if path in ("/engine","/engine/"):
        request.scope["path"]="/health"; request.scope["raw_path"]=b"/health"
    elif path.startswith("/engine/"):
        mapped=path[len("/engine"):]; request.scope["path"]=mapped; request.scope["raw_path"]=mapped.encode()
    response=await call_next(request)
    if path.startswith("/engine"): response.headers["X-THESIS-Engine"]="online"
    return response

@app.get("/runtime/health",include_in_schema=False)
def runtime_health():
    return {"ok":True,"runtime":"THESIS","gateway":"/engine/*","wallet_identity":"v1","cli":["thesis","monadbuilder"],"synthetic_users":{"personas":"/synthetic/personas","status":"/synthetic/status","run":"/synthetic/run"}}

_ROOT=Path(__file__).resolve().parents[2]; _WEB_DIST=_ROOT/"web"/"dist"
if _WEB_DIST.exists():
    ...
    def spa(path:str):
        ...
        if candidate.is_file() and _WEB_DIST.resolve() in candidate.parents: return FileResponse(candidate)
