...
contract AgentRegistry {
    ...
    function register(address agent, bytes32 identity, bytes32 capabilities, uint64 expiresAt)
        external
    {
    ...
    function revoke(address agent) external {
        agents[msg.sender][agent].active = false;
        emit AgentRevoked(msg.sender, agent);
    }

    function setReputation(address agent, uint32 reputation) external {
        if (!agents[msg.sender][agent].active) revert ThesisErrors.NotFound();
        agents[msg.sender][agent].reputation = reputation;
        emit AgentReputation(msg.sender, agent, reputation);
    }

    function authorized(address owner, address agent) external view returns (bool) {
        ThesisTypes.AgentProfile memory a = agents[owner][agent];
        return a.active && a.expiresAt > block.timestamp;
    }

    function hasCapability(address owner, address agent, bytes32 capabilityBit)
        external
        view
        returns (bool)
    {
        ThesisTypes.AgentProfile memory a = agents[owner][agent];
        if (!a.active || a.expiresAt <= block.timestamp) return false;
        // capabilities is a bitmask packed in bytes32
        return (uint256(a.capabilities) & uint256(capabilityBit)) != 0;
    }

    function listAgents(address owner) external view returns (address[] memory) {
        return _agentList[owner];
    }

    function getAgent(address owner, address agent)
        external
        view
        returns (ThesisTypes.AgentProfile memory)
    {
        return agents[owner][agent];
    }
}
