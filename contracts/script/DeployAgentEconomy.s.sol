...
contract DeployAgentEconomy is Script {
    function run() external {
        ...
        AgentServiceMarket market = new AgentServiceMarket(receiptAddress, governor, treasury, uint16(feeBps), uint32(reviewPeriod));
        if (deployedReceiptChain || configureExistingReceiptChain) ReceiptChain(receiptAddress).setSealer(address(market), true);
        vm.stopBroadcast();
